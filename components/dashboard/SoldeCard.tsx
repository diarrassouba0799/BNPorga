...
export default function SoldeCard({ compte }: { compte: Compte }) {
  ...
  return (
    <div className="bg-gradient-to-br from-[#007A4D] via-[#006B43] to-[#005A38] rounded-2xl p-6 text-white shadow-xl">
      
      <div className="flex items-center justify-between mb-5">
        <div>
          <p className="text-white/80 text-sm font-medium">{compte.libelle}</p>
      ...
      <div className="flex items-center gap-2 bg-white/10 rounded-lg px-3 py-2 w-fit backdrop-blur-sm">
        <TrendingUp size={14} />
        <span className="text-xs text-white/80">+2 800,00 € ce mois</span>
      </div>
    </div>
  )
}