...
export default function SoldeCardDynamic() {
  ...
  const libelleCompte = langue === 'it' ? 'Conto Corrente' : compte.libelle

  return (
    <div
      className="relative rounded-2xl p-6 text-white overflow-hidden"
      style={{
        background: 'linear-gradient(135deg, #004a00 0%, #006635 40%, #009B4E 100%)',
        boxShadow: '0 8px 32px rgba(0,100,50,0.35)',
      }}
    >
      ...
      <div className="relative z-10">
        <div className="flex items-center justify-between mb-4">
          <div>
            <p className="text-green-100 text-sm font-medium">{libelleCompte}</p>
        ...
        <div className="mb-5">
          <p className="text-green-100 text-xs mb-1">{t('solde_disponible', langue)}</p>
        ...
        <div className="flex items-center gap-2 bg-white/10 rounded-lg px-3 py-2 w-fit">
          <TrendingUp size={14} />
          <span className="text-xs font-medium">{t('ce_mois', langue)}</span>
        </div>
      </div>
    </div>
  )
